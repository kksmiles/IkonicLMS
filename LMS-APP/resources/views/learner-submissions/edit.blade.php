...
@section('title')
    {{ $assignment->title }} : Submission form
@endsection
...
<div class="mt-8">
    <div class="mt-4">
        <div class="p-6 bg-white rounded-md shadow-md">
            <h2 class="text-xl text-gray-700 font-semibold capitalize">Update assignment</h2>
            <form action="{{ route('learner-submissions.update', $learnerSubmission->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-4">
                    <input type="hidden" name="assignment_id" value="{{ $assignment->id }}">
                    <div>
                        <label class="text-gray-700" for="image">File</label>
                        <input name="submission_file" class="form-input w-full mt-2 rounded-md focus:border-indigo-600" type="file">
                    </div>                    
                </div>
