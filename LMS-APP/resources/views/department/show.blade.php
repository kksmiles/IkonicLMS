...
<div class="w-full lg:flex p-5 bg-white mt-5 rounded-md shadow-md">
    <div class="w-full lg:w-3/12 flex-none">
        <div class="w-full h-64 bg-cover rounded-md text-center overflow-hidden" style="background-image: url('{{ asset($department->getImageURL()) }}')" title="Course Image">
        </div>
    ...
    <div class="p-1 flex flex-col justify-between leading-normal ml-0 lg:ml-8">
        <div class="mb-8 mt-3 lg:mt-0">
            <div class="text-black font-bold text-xl mb-2">{{ $department->name }}</div>
            ...
            <table class="min-w-full">
                <thead>
                    <tr>
                        ...
                        <th class="px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">department/Department</th>
                ...
                <tbody class="bg-white">
                    @foreach($department->instructors as $user)
                    <tr>
                                        ...
                                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                            ...
                                            <div class="mt-5">
                                                <p class="text-sm leading-5 text-gray-500">
                                                    Are you sure you want to remove {{ $user->full_name }} from {{ $department->name }}?
                                                </p>
                                            </div>
                                        </div>
                        ...
                        <form id="remove-form{{ $user->id }}" action="{{ route('department-user.detach') }}" method="POST" class="hidden">
                            @csrf
...
<div class="flex justify-end mt-4">
    <a href="{{ route('department-user.add', $department->id) }}">
