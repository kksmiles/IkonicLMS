...
<div class="p-6 bg-white rounded-md shadow-md">
    ...
    <h3 class="text-blue-500 text-md mt-8"> 
        ...
        @endif
        <a href="{{ route('courses.show', $course_material->course_material_topic->course->id) }}"> 
            {{ $course_material->course_material_topic->course->title }} 
        </a> / 
        <a href="{{ route('courses.show', $course_material->course_material_topic->course->id) }}"> 
            {{ $course_material->course_material_topic->course->title }} 
        </a> /
            ...
            <form action="{{ route('course-materials.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-4">
                    ...
                    <div class="relative">
                        ...
                        <select class="form-input w-full mt-2 rounded-md focus:border-indigo-600" name="course_material_topic_id" id="course_material_topic_id" required>
                            @foreach($course_material_topics as $course_material_topic)
                                <option value="{{ $course_material_topic->id }}" {{ ($course_material_topic->id==$course_material->course_material_topic_id) ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input name="title" class="form-input w-full mt-2 rounded-md focus:border-indigo-600" type="text" value="{{ $course_material->title }}" required>
                    </div>
                    ...
                    <div>
                        ...
                        <input name="course_material_file" class="form-input w-full mt-2 rounded-md focus:border-indigo-600" type="file">
                    </div>                    
                </div>
