...
<form action="{{ route('course-learner.store', $course->id) }}" method="POST">
    @csrf
                ...
                <tbody class="bg-white">
                    ...
                    <tr>
                        <td class="px-1 py-4 whitespace-no-wrap border-b border-gray-200">
                            <input type="checkbox" name="users[]" value="{{ $user->id }}" class="ml-3 form-checkbox h-5 w-5 text-indigo-600 border-gray-700" {{ in_array($user->id, $course->learners->pluck('id')->toArray()) ? 'checked' : '' }}>
                        </td>
