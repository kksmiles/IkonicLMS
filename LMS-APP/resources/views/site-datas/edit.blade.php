...
<div class="mt-8">
    <div class="mt-4">
        <div class="p-6 bg-white rounded-md shadow-md">
            ...
            <form action="{{ route('site-datas.update', $siteData->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-4">
                    <div>
                        <label class="text-gray-700" for="title">Title</label>
                        <input name="title" value="{{ $siteData->title }}" class="form-input w-full mt-2 rounded-md focus:border-indigo-600" type="text" required>
                    </div>
